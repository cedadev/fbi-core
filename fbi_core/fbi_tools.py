...
def archive_summary(
    path, max_types=5, max_vars=1000, max_exts=10, include_removed=False, **kwargs
):
...
def fbi_listdir(
    directory, fetch_size=10000, dirs_only=False, removed=False, hidden=True
):
...
def get_record(path):
    try:
        record = es.get_source(
            index=indexname, id=_create_id(path), request_timeout=100
        )
    except elasticsearch.exceptions.NotFoundError:
        return None
    return dict(record)
...
def bulk_update(records):
    ...
    for record in records:
        body += (
            json.dumps(
                {"update": {"_index": indexname, "_id": _create_id(record["path"])}}
            )
            + "\n"
        )
...
def update_file_location(path_list, location):
    ...
    for path in path_list:
        ...
        if rec is None:
            raise (
                ValueError(
                    f"No FBI record for path {path} so can't change to {location}"
                )
            )
...
def parameters(directory, removed=False, hidden=False):
    ...
    parameter_dict = {}
    cf_units_pattern = re.compile(r"(.*?)\s+[Ss]ince")
        ...
        for phen in phenomena:
            if phen["agg_string"] == agg_string:
                ...
                if var_id is not None:
                    parameter_dict[para_key]["var_id"] = var_id
                if standard_name is not None:
                    parameter_dict[para_key]["standard_name"] = standard_name
                if long_name is not None:
                    parameter_dict[para_key]["long_name"] = long_name
                if units is not None:
                    parameter_dict[para_key]["units"] = units
